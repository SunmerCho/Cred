...
export default function App() {
  ...
  return (
    <View style={styles.container}>
      ...
      <Header step={'3'} title={'/3    Your finances'} progress={18 / 26} display={'flex'} />

      <Text style={styles.title_text}>How much do you pay each month on credit repayments?</Text>

      <TextFiled style={styles.input} label="Amount (£)" type='numeric' onChange={(text) => setInput(text)} />

      <ButtonOrange style={styles.btn} title='Next' opacity={input == '' ? 0.5 : 1} onPress={() => { input == '' ? null : router.push('/organic/Utilities') }} />
    </View>
  );
}

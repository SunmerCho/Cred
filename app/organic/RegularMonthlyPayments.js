...
export default function App() {
  ...
  return (
    <View style={styles.container}>
      ...
      <Header step={'3'} title={'/3    Your finances'} progress={24 / 26} display={'flex'} />

      <Text style={styles.title_text}>What are your other regular monthly payments?</Text>

      <TextFiled style={styles.input} label="Amount (£)" type='numeric' onChange={(text) => setInput(text)} />

      <ButtonOrange style={styles.btn} title='Next' opacity={input == '' ? 0.5 : 1} onPress={() => { input == '' ? null : router.push('/organic/IncomeDrop') }} />
    </View>
  );
}
